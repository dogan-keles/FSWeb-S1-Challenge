...
function indekseGoreFenomen(fenomenlerDizisi, indeksimiz) {
 const fenomenlerArr = fenomenlerDizisi[indeksimiz]
  return `${indeksimiz}. indekste bulunan fenomen: ${fenomenlerArr.profile}`
}
...
function fenomenSil(dizi, indeksimiz) {
  let yeniDizimiz = [...dizi] ; 
  yeniDizimiz.splice(indeksimiz, 1);
  return yeniDizimiz;
}
...
function fenomenEkle(fenomenlerimiz, sıraNo, profilIsmi, takipci, gonderiSayisi, platformAdi) {
  let fenomenlerCopy = [...fenomenlerimiz];
  const newObject = {
    "number": sıraNo,
    "profile": profilIsmi,
    "followers": takipci,
  ...
  fenomenlerCopy.push(newObject);
  return fenomenlerCopy ;
}
...
function enFenomenler(fenomenler) {
  let populerFenomenler = [];
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].followers > 100000000){
      populerFenomenler.push(fenomenler[i].profile)
    }
  }
  return populerFenomenler;
}
...
console.log(fenomenGonderimSayisi(fenomenler, 'Will Smiths'))
//console.log(fenomenGonderimSayisi(fenomenler, 'Johhny Cash'))
